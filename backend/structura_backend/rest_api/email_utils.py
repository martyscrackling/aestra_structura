...
def send_phase_update_notification_email(
    *,
    to_email: str,
    ...
    """Notify a project client that the supervisor submitted a phase update."""

    app_name = getattr(settings, "APP_NAME", "Structura")
    frontend_url = (getattr(settings, "FRONTEND_URL", "") or "").strip().rstrip("/")

    client_first_name = (client_first_name or "").strip()
    project_name = (project_name or "").strip()
    phase_name = (phase_name or "").strip()
    subtask_title = (subtask_title or "").strip()
    subtask_status = (subtask_status or "").strip()
    progress_notes = (progress_notes or "").strip()
    supervisor_name = (supervisor_name or "").strip() or "Supervisor"

    subject_parts = ["Phase update submitted"]
    if project_name:
        subject_parts.append(f"({project_name})")
    subject = " ".join(subject_parts)

    greeting = f"Hi {client_first_name}," if client_first_name else "Hi,"
    lines: list[str] = [
        greeting,
        "",
        f"{supervisor_name} submitted a project update.",
    ]
    ...
    if subtask_title:
        lines.append(f"Subtask: {subtask_title}")
    if subtask_status:
        lines.append(f"Status: {subtask_status}")

    lines.append(f"Photo attached: {'Yes' if has_photo else 'No'}")
...
def send_phase_update_summary_email(
    *,
    to_email: str,
    client_first_name: str | None = None,
    project_name: str | None = None,
    ...
    """Notify client with a consolidated phase update summary."""
    ...
    supervisor_name = (supervisor_name or "").strip() or "Supervisor"
    progress_notes = (progress_notes or "").strip()
    subtask_lines = [line for line in (subtask_lines or []) if (line or '').strip()]

    subject_parts = ["Phase update summary"]
    if project_name:
        subject_parts.append(f"({project_name})")
    subject = " ".join(subject_parts)
    ...
    lines: list[str] = [
        greeting,
        "",
        f"{supervisor_name} submitted a phase update.",
    ]
    ...
    if subtask_lines:
        lines.extend(["", "Updated subtasks:"])
        lines.extend([f"- {line}" for line in subtask_lines])
    else:
        lines.extend(["", "Updated subtasks: (not specified)"])
